...
let #Even tau
    ...
    p1=plot(datatau2s.a, datatau2s.N1, color="#73D055FF", label="τ=2.0")
    plot!(datatau2ua.a[20:end], datatau2ua.N1[20:end], color="#73D055FF", linestyle=:dash, lw=1.5, label="")
    ...
    plot!(datatau6ua.a[40:end], datatau6ua.N1[40:end], color="#404788FF", linestyle=:dash, lw=1.5, label="")
    ...
    ylims!(-0.5, 20.0)
    xlims!(0.0, 5.0)
    plot!(legend=false, left_margin=5mm)
    p2=plot(datatau2s.a, datatau2s.N1, color="#73D055FF", label="τ=2.0")
    plot!(datatau2ua.a[20:end], datatau2ua.N1[20:end], color="#73D055FF", linestyle=:dash, lw=1.5, label="")
    plot!(datatau4s.a, datatau4s.N1, color="#1F968BFF", label="τ=4.0")
    ...
    p3=plot(datatau6s.a, datatau6s.N1, color="#404788FF", label="τ=6.0")
    plot!(datatau6ua.a[30:end], datatau6ua.N1[30:end], color="#404788FF", linestyle=:dash, lw=1.5, label="")
    plot!(maxmindata6.a, maxmindata6.maximum, color="#404788FF", linestyle=:dashdotdot,lw=2, label="")
    plot!(maxmindata6.a, maxmindata6.minimum, color="#404788FF", linestyle=:dashdotdot,lw=2, label="")
    plot!([32, 32], [-0.5, findRCvalue(datatau6, 32.0, "stable")], color=:black, linestyle=:solid, lw=1, label="")
    plot!([33, 33], [-0.5, findRCvalue(maxmindata6, 33.0, "max")], color=:black, linestyle=:solid, lw=1, label="")
    xlabel!("a")
    ylims!(-0.5, 20.0)
    xlims!(30.0, 36.0)
    xticks!([30, 32, 34, 36])
    plot!(legend=false,yaxis=false, left_margin=-5mm)
    plot(p1, p2,p3, layout = @layout([a{0.2w} b{0.6w} c{0.2w}]), size = (800, 400), bottom_margin=5mm)
...
let #tau=3 (odd)
    ...
    xticks!([10, 11])
    ylims!(-0.5, 30.0)
    plot!([11.04, 11.04], [-0.5, findRCvalue(datatau3u3[1], 11.04, "unstable")], color=:black, linestyle=:solid, lw=1, label="")
    plot!([10.92, 10.92], [-0.5, findRCvalue(datatau3s3p4upperupper, 10.92, "stable")], color=:black, linestyle=:solid, lw=1, label="")
    plot!(yaxis=false, left_margin=-5mm)
    plot(p1, p2, layout = @layout([a{0.2w} b{0.8w}]), size = (800, 400), bottom_margin=5mm)
...
let #tau=5 (odd)
    ...
    plot!(datatau5u0.a, datatau5u0.N1, color=:black, linestyle=:dash, lw=1.5, label="")
    plot!([20.4, 20.4], [-0.5, findRCvalue(datatau5s2pupper, 20.4, "stable")], color=:black, linestyle=:solid, lw=1, label="")
    plot!([20.7, 20.7], [-0.5, findRCvalue(datatau5u2pupper, 20.7, "unstable")], color=:black, linestyle=:solid, lw=1, label="")
    xlabel!("a")
