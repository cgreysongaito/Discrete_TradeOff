function plotxppaut4p3(varargin)
...
colorGreen = 'g'; colorCyan = 'c'; colorOlive = [.7 .7 0];
LineDefaultCell = {
...
info = struct('Data',[],'Defaults',defaults,...
...
for jline=1:numel(info.LineData)
    ...
    options = info.LineOptions(jop);
    x = lineData.x;
    y = lineData.y;
    y2 = lineData.y2;
    hp = plot(x,y,x,y2,'k-','linewidth',1);
    hp = SetMarkerIndices(x,y,y2,options,hp);
...
%% ====SET MARKER INDICIES====
function hp=SetMarkerIndices(x,y,y2,options,hp)
NrPts = options.NrPts;
LineStyle = options.LineStyle;
MarkerIndices = 1:NrPts:numel(x);
ver = version(); % MATLAB version number
nver = str2double(ver(1:3));
if nver>=9.1 % vflag=1 if MATLAB is 9.1(R2016b) or Newer
    for jhp=1:numel(hp)
...
options = struct('Type',[],'jlines',[]);
LineOptions = repmat(options,1,numtypes);
for jop=1:numtypes
    type = types{jop};
...
%% ====MAKE LEGEND====
function MakeLegend(options,data)
numop = numel(options);
figs = NaN(1,numop);
names = cell(1,numop);
jfig = 0;
...
if strcmp(status,'START')
    ...
    posbox = [3 1.5 6.5 1.2];
    %%-- Set Buttons
    strs = {'Quit','Help'};
    for jstr=1:numel(strs)
        str = strs{jstr};
        posbox = posbox + (jstr>1)*[7.5 0 0 0];
        uicontrol('Style','pushbutton','String',str,...
            'Units','characters','Position',posbox,...
...
if strcmp(status,'NEW')
    clf(figPrompt);
    str = sprintf('Setup for %s',info.Filename);
    set(figPrompt,'Name',str);
    posPrompt = figPrompt.Position;
...
%% Set Buttons
jui = 0;
hd = {'Quit','Reset','Black','Load','Help'};
if strcmp(status,'NEW')
    posbox0 = [3 1.5 6.5 1.2];
    for jh=1:numel(hd)
        jui = jui + 1;
        str = hd{jh};
        posbox = posbox0 + (jh-1)*[7.5 0 0 0];
...
elseif strcmp(status,'UPDATE')
    for jh=1:numel(hd)
        jui = jui + 1;
        str = hd{jh};
        set(info.hui{jui},'Callback',@(s,e)setChoice(info,str));
    end
end
...
options = info.LineOptions(focus);
% Set LineStyle
jui = jui + 1;
list = info.Defaults.LineStyleList;
val = find(strcmp(list,options.LineStyle),1);
if strcmp(status,'NEW')
    vspace = vspace + 2;
...
% Set LineWidth
jui = jui + 1;
strbox = num2str(options.LineWidth);
if strcmp(status,'NEW')
    vspace = vspace + 2;
    postxt = [3 coord_top-vspace 16 1];
...
val = find(strcmp(list,options.Marker),1);
if strcmp(status,'NEW')
    vspace = vspace + 2;
    postxt = [3 coord_top-vspace 12 1];
...
% Set MarkerSize
jui = jui + 1;
strbox = num2str(options.MarkerSize);
if strcmp(status,'NEW')
    vspace = vspace + 2;
    postxt = [3 coord_top-vspace 12 1];
...
if strcmp(status,'NEW')
    ...
    uicontrol('Style','text','String',strtxt,...
        'HorizontalAlignment','left','Units','characters','Position',postxt);
    posbox = [20 coord_top-vspace 11 1.2];
    info.hui{jui} = uicontrol('Style','edit','String',strbox,...
        'HorizontalAlignment','left','Units','characters','Position',posbox);
elseif strcmp(status,'UPDATE')
    set(info.hui{jui},'String',strbox,...
...
for jline=1:numlines
    ...
    ncopy = lines.ncopy;
    if ncopy==1
        strbox = sprintf('[%d,%d,%d] (%s)',lines.Code,options.Legend);
    else
        strbox = sprintf('[%d,%d,%d] (%s)(%d)',lines.Code,options.Legend,ncopy-1);
    end
    hp = lines.hp;
...
switch opt
    ...
    case 'NrPts'
        ...
        jlines = find(arrayfun(@(c)isequal(type,c.Code([1,3])),info.LineData));
        for jline=jlines
            x = info.LineData(jline).x;
            y = info.LineData(jline).y;
            y2 = info.LineData(jline).y2;
            hp = info.LineData(jline).hp;
            hp = SetMarkerIndices(x,y,y2,options,hp);
            info.LineData(jline).hp = hp;
        end
    ...
    case {'TruncTol','LenTol'}
        ...
        if strcmp(opt,'TruncTol')
            val = str2double(str);
            if ~(val>0)
                str = sprintf('%s must be a positive number.',opt);
                myPrompt(info,'UPDATE');
                msgbox(str,'');
                return;
            end
    ...
    case 'Load'
        ...
        [filename,path] = uigetfile('*.dat',str);
        fp = fopen([path filename],'r');
        data = fscanf(fp,'%f',[6,inf]);
        fclose(fp);
        if ~isempty(info.Data)
            fkeeps = {'Data','Defaults','nfigPlot','nfigPrompt'};
            str = 'Discard the current diagram?';
            stitle = 'Options for Loading';
            soptions = {'Keep','Discard','Cancel'};
            sdefault = 'Discard';
            choice  = questdlg(str,stitle,soptions{:},sdefault);
    ...
    case 'Reset'
        fnames = fieldnames(info)';
        fkeeps = {'Data','Defaults','nfigPlot','nfigPrompt','Filename'};
        fdel = setdiff(fnames,fkeeps);
        for ch=fdel
            info.(ch{1}) = [];
        end
